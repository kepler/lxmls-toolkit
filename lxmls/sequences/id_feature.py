...
class IDFeatures(object):
    ...
    def get_sequence_features(self, sequence):
        ...
        features = []
        features = self.add_initial_features(sequence.y[0], features)
        ...
        features = []
        features = self.add_final_features(sequence.y[-1], features)
    ...
    def get_initial_features(self, sequence, y):
        if y not in self.initial_state_feature_cache:
            edge_idx = []
            edge_idx = self.add_initial_features(y, edge_idx)
    ...
    def add_final_features(self, y_prev, features):
        # Get label name from ID.
