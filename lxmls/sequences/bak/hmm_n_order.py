...
class HMM_N_Order:
    def __init__(self, dataset, true_states=-1, order=2):
        ...
        self.all_states = tuple([tuple(x) for x in self.all_states])
    ...
    @staticmethod
    def add_counts(strucuture, state, history, value):
        if state == 42:
    ...
    @staticmethod
    def set_counts(strucuture, state, history, value):
        if history not in strucuture:
            strucuture[history] = {}
        if state in strucuture[history]:
            strucuture[history][state] = value
    ...
    @staticmethod
    def get_counts(strucuture, state, history):
        if history not in strucuture:
            return 0.0
        if state in strucuture[history]:
    ...
    def clear_counts(self, obs_smoothing=0.0, trans_smoothing=0.0):
        ...
        if obs_smoothing != 0.0:
            ...
            for tt in range(self.true_states):
                if tt not in self.observation_counts:
                    self.observation_counts[tt] = {}
                    for w_id in range(self.nr_types):
        ...
        if trans_smoothing != 0.0:
            ...
            for hmm_s in self.all_states:
                ...
                for tt in range(self.base_states):
    ...
    ####
    @staticmethod
    def normalize_counts(count_table):
        param_table = {}
        for condition, values in list(count_table.items()):
    ...
    def sanity_check_counts(self, seq_list):
        ...
        nr_tokens = sum([len(seq.x) for seq in seq_list.seq_list])
    ...
    @staticmethod
    def sanity_check_fb(forward, backward):
        H, N = forward.shape
    ...
    @staticmethod
    def get_node_posteriors_aux(seq, forward, backward, likelihood):
        H, N = forward.shape
    ...
    @staticmethod
    def evaluate_corpus(seq_list, predictions):
        total = 0.0
        correct = 0.0
