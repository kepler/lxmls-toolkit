...
class DependencyFeatures(object):
    ...
    def create_arc_features(self, instance, h, m, add=False):
        ...
        if self.use_lexical:
            ...
            f = self.lookup_fid(
                "{0}_{1}_{2}_{3}_{4}".format(k, instance.words[h], instance.pos[h], instance.words[m], instance.pos[m]),
                add)
            ff.append(f)
            k += 1
        ...
        if self.use_contextual:
            ...
            f = self.lookup_fid(
                "{0}_{1}_{2}_{3}_{4}_{5}".format(k, instance.pos[h], hpp, hpn, instance.pos[m], att_dir), add)
            ff.append(f)
            k += 1
            ...
            f = self.lookup_fid(
                "{0}_{1}_{2}_{3}_{4}_{5}".format(k, instance.pos[h], instance.pos[m], mpp, mpn, att_dir), add)
            ff.append(f)
            k += 1
            # Head pos+posl, modifier pos+posl, dir
            f = self.lookup_fid(
                "{0}_{1}_{2}_{3}_{4}_{5}".format(k, instance.pos[h], hpp, instance.pos[m], mpp, att_dir), add)
            ff.append(f)
            k += 1
            # Head pos+posl, modifier pos+posr, dir
            f = self.lookup_fid(
                "{0}_{1}_{2}_{3}_{4}_{5}".format(k, instance.pos[h], hpp, instance.pos[m], mpn, att_dir), add)
            ff.append(f)
            k += 1
            # Head pos+posr, modifier pos+posl, dir
            f = self.lookup_fid(
                "{0}_{1}_{2}_{3}_{4}_{5}".format(k, instance.pos[h], hpn, instance.pos[m], mpp, att_dir), add)
            ff.append(f)
            k += 1
            # Head pos+posr, modifier pos+posr, dir
            f = self.lookup_fid(
                "{0}_{1}_{2}_{3}_{4}_{5}".format(k, instance.pos[h], hpn, instance.pos[m], mpn, att_dir), add)
            ff.append(f)
            k += 1
            # Head pos+posl, modifier pos+posl+posr, dir
            f = self.lookup_fid(
                "{0}_{1}_{2}_{3}_{4}_{5}_{6}".format(k, instance.pos[h], hpp, instance.pos[m], mpp, mpn, att_dir), add)
            ff.append(f)
            k += 1
            # Head pos+posr, modifier pos+posl+posr, dir
            f = self.lookup_fid(
                "{0}_{1}_{2}_{3}_{4}_{5}_{6}".format(k, instance.pos[h], hpn, instance.pos[m], mpp, mpn, att_dir), add)
            ff.append(f)
            k += 1
            # Head pos+posl+posr, modifier pos+posl, dir
            f = self.lookup_fid(
                "{0}_{1}_{2}_{3}_{4}_{5}_{6}".format(k, instance.pos[h], hpp, hpn, instance.pos[m], mpp, att_dir), add)
            ff.append(f)
            k += 1
            # Head pos+posl+posr, modifier pos+posr, dir
            f = self.lookup_fid(
                "{0}_{1}_{2}_{3}_{4}_{5}_{6}".format(k, instance.pos[h], hpp, hpn, instance.pos[m], mpn, att_dir), add)
            ff.append(f)
            k += 1
            # Head pos+posl+posr, modifier pos+posl+posr, dir
            f = self.lookup_fid(
                "{0}_{1}_{2}_{3}_{4}_{5}_{6}_{7}".format(k, instance.pos[h], hpp, hpn, instance.pos[m], mpp, mpn,
                                                         att_dir), add)
            ff.append(f)
            k += 1
    ...
    @staticmethod
    def compute_scores(feats, weights):
